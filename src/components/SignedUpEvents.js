...
const SignedUpEvents = (props) => {
    ...
    const signOutFromEvent = async (uid, newRoles, index) => {
        ...
        await getDoc(uid).then((e) => {
            ...
            cloneObj.forEach((r) => {
                if (r[0] === updateRole) {
                    r[1] = r[1].filter((fc) => fc.id !== user.currentUser.uid)
                    updateFlag = true;
                }
            })
                ...
                e.roles.forEach((eventNeeds, index1, array) => { // Each role in event
                    ...
                    if (userRoles.some((r) => r === eventNeeds.substring(0, 3))) {
                        if (Object.entries(e.users).some((g1) => {
                            ...
                            addToList = true;
                            result.push(<p key={index1}>
                                הינך רשום כ - {categories[cat][role]}
                                <button id="index" className="btn--accent" onClick={() => signOutFromEvent(e.uid, e.roles, index1)}>הסרה מאירוע</button>
                            </p>)
                        }
                    }
                });
