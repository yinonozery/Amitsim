...
const NewCall = () => {
  ...
  const callRoles = (callRole) => {
    ...
    if (!callStringsRoles.includes(callRole)) callStringsRoles.push(callRole);
    else callStringsRoles.splice(callStringsRoles.indexOf(callRole), 1);
  };
    ...
    return (
      <div className="role" key={indexRole}>
        <input
          ...
          value={indexRole}
          id={`0-${indexRole}`}
          onChange={() => callRoles(`0-${indexRole}`)}
        />
        ...
        <div className="box box--sub">
          <form onSubmit={handleSubmit}>
            ...
            <div id="rolesList" className="container">
              <div className="category">{showCats}</div>
            </div>
