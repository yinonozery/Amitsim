...
const UsersTable = (props) => {
    ...
    function createData(name, city, mail, gender, phone, ssn, status, isAdmin, activities) {
        return { name, city, mail, gender, phone, ssn, status, isAdmin, activities };
    }
    ...
    const searchRes = users.filter((user) => {
        ...
        if (searchWord.length < 2)
            return true;
        return user.city.includes(props.search) ||
            user.name.includes(props.search) ||
            user.email.includes(props.search) ||
            user.tel.includes(props.search) ||
            user.ssn.includes(props.search)
            ;
    });

    const rows = searchRes.map((user) => {
        ...
        const activities = <div>
            <button className="form__btn btn--accent"
                onClick={() => deActive(user.uid)}
            >
                {toggle}
            </button>
        </div>
        return createData(user.name, user.city, user.email, user.gender, user.tel, user.ssn, activeImage, isAdminImage, activities);
    })
